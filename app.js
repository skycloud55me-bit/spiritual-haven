...
(function(){
    ...
    if (prayerReminders) {
      const prayerTimes = [
        "🌄 الصبح: أفضل وقت عند الفجر",
         "☀️ الظهر: عندما  تكون الشمس فالمنتصف",
  ...
  const prophetStories = [
    {
      ...
      moral: "لعبادة الله و اعمار الارض"
    },
    {
      title: "نبي الله نوح",
  ...
  function initDailyQuiz(){
    ...
    form.addEventListener('submit', function(ev){ 
      ...
      if(percent === 100) {
        ...
        result.innerHTML += '<div>جيد جدًا — أحسنت!</div>';
        awardPoints(10, "نتيجة جيدة في الاختبار");
      } else {
        result.innerHTML += '<div>حاول مرة أخرى غدًا — التعلم مستمر</div>';
  ...
  document.getElementById('joinChallenge')?.addEventListener('click', ()=>{ 
    ...
    showTempToast('انضممت للتحدي الأسبوعي — تابع تقدمك!');
  });
