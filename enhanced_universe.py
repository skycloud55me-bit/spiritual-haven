...
quran_verses = [
    ...
    {"verse": "وَنُنَزِّلُ مِنَ الْقُرْآنِ مَا هُوَ شِفَاءٌ وَرَحْمَةٌ لِلْمُؤْمِنِينَ", "surah": "الإسراء - 82", "explanation": "القرآن شفاء ورحمة للمؤمنين."},
    {"verse": "وَتَوَكَّلْ عَلَى الْحَيِّ الَّذِي لَا يَمُوتُ", "surah": "الفرقان - 58", "explanation": "التوكّل على الله الحيّ الذي لا يموت."}
]
...
islamic_questions = [
    ...
    {"question": "من هو النبي الذي سُمّي خليلَ الله؟", "options": ["موسى", "إبراهيم", "محمد"], "answer": "إبراهيم", "category": "الأنبياء"},
...
# الصفحة 3 الجديدة: جنات الطاعة
    ...
    with tab3:
        ...
        recommended_acts = [
            ...
            {"act": "صيام الاثنين والخميس", "reward": "تكفير للذنوب ورفعة في الدرجات"},
...
# الصفحة 4: حديقة التأمل
elif page == "💭 حديقة التأمل":
    st.markdown('<div class="islamic-card">', unsafe_allow_html=True)
    st.markdown('<h2 class="blessed-text">💭 حديقة التأمل</h2>', unsafe_allow_html=True)
    
    st.subheader("📝 تأملاتك اليومية")
    reflection = st.text_area("اكتبي تأملاتك الروحانية لهذا اليوم:")
    
    if st.button("💾 سجلي التأمل"):
        if reflection:
            st.session_state.enhanced_data['reflection_entries'].append({
                'date': datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                'text': reflection
            })
            st.success("تم حفظ تأملاتك! 🌿")
        else:
            st.warning("اكتبي شيئاً قبل التسجيل!")
    
    st.subheader("📜 سجل تأملاتك السابقة")
    for entry in st.session_state.enhanced_data['reflection_entries']:
        st.write(f"**{entry['date']}**: {entry['text']}")
    
    st.markdown('</div>', unsafe_allow_html=True)

# الصفحة 5: شجرة الامتنان
elif page == "🌺 شجرة الامتنان":
    st.markdown('<div class="islamic-card">', unsafe_allow_html=True)
    st.markdown('<h2 class="blessed-text">🌺 شجرة الامتنان</h2>', unsafe_allow_html=True)
    
    st.subheader("🙏 ما الذي تشكرين الله عليه اليوم؟")
    gratitude = st.text_area("اكتبي ثلاثة أشياء تشكرين الله عليها اليوم:")
    
    if st.button("🌟 سجلي الامتنان"):
        if gratitude:
            today = datetime.now().date().strftime("%Y-%m-%d")
            if today not in st.session_state.enhanced_data['gratitude_days']:
                st.session_state.enhanced_data['gratitude_days'].append(today)
                st.session_state.enhanced_data['good_deeds'] += 1
                st.success("ماشاء الله! شكركِ لله يقربكِ إليه 🌟")
                st.balloons()
            else:
                st.info("لقد سجلتِ امتنانك لهذا اليوم بالفعل!")
        else:
            st.warning("اكتبي شيئاً تشكرين الله عليه!")
